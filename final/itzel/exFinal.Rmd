...
Esta función calcula la suma de las ganancias de los 20 juegos de la ruleta
```{r}
juegos20 <- function(){
...
prob.gan.pos <- pos/100 # probabilidad aproximada de que la ganancia total de los 20 juegos sea positiva
prob.gan.pos
```

### b)

X se distribuye binomial con parámetros n = 20 (total de juegos) 
y p = 18/38 (probabilidad de ganar $5)

La probabibilidad de que la ganancia total sea positiva es igual a la probabilidad
de que gane $5 en 11 o más tiros de la ruleta (de los 20 en total), que es igual
a 1 menos la probabilidad de que gane $5 en 10 o menos tiros de la ruleta (de los 20)
...
tab.P <- cbind(hist.P$breaks[-n],hist.P$breaks[-1],hist.P$counts,hist.P$counts/sum(hist.P$counts), cumsum(hist.P$counts),cumsum(hist.P$counts/sum(hist.P$counts)))
...
Como u ~ U(0,5) => A debe de ser menor o igual a 1, de hecho A debe de ser un número pequeño para así poder generar también valores pequeños de u, como no hay grandes diferencias entre tomar n?meros menores a 1/2 y tomar 1/2, se elige como ?ptimo A = 1/2 (valores menores realizan muchas iteraciones y toman mucho tiempo en generar la muestra)
```{r}
A <- 1/2
```
...
Se observa que todas las variables están muy correlacionadas pues todas (a excepción de mec y sta) tienen una correlación cercana a .5 o mayor a .5; hay que mencionar también que todas tienen correlación positiva. En la matriz de dispersión se observa que hay variables en las cuales resalta su dependencia lineal, por ejemplo vec y alg, mientras que hay otras en las que no, por ejemplo mec y sta.
...
Obtener los intervalos de confianza para la correlación
...
La probabilidad obtenida es aproximadamente 0.169 que es muy cercana a la exacta (0.1690) y en general es mejor aproximaci?n que la obtenida en el inciso b)
...
Se utiliza un muestreador Metrópolis pues ambas distribuciones prior son simétricas
...
metropolis2 <- function(theta,x0,N){
  ...
  k<-0
  for(i in 2:N){
    j <- sample(10:990,size =1, replace =TRUE) #obtener valores "aleatorios" de la función Jeffrey
...
```{r}
x.plot <- seq(0,100)
plot(x.plot,dunif(x.plot),type="l",xlab="theta",xlim= c(0,1),ylim=c(0,1.5),main="Distribución prior uniforme")
...
En ambos casos se utilizó una cadena de tamaño 1950, con un valor de theta de 1/2

Error estándar asociado a montecarlo
```{r}
...
Gráficas de autocorrelación para cada prior
```{r}
par(mfrow=c(1,2))
...
bm(bin.unif$x[burn.in:N])
```
Su valor promedio es aproximadamente 5.5 =n*theta, por lo tanto el valor promedio de theta es aproximadamente 5.5/n = 5.5/10 =.55

```{r}
bm(bin.jeff$x[burn.in2:N])
```

Su valor promedio es aproximadamente 3.8 =n*theta, por lo tanto el valor promedio de theta es aproximadamente 3.8/n = 3.8/10 =.38
...
bm(bin.unif$x[burn.in:N])
```
Su valor promedio es aproximadamente 0 =n*theta, por lo tanto el valor promedio de theta es aproximadamente 0/n = 0/10 = 0

```{r}
bm(bin.jeff$x[burn.in2:N])
```

Su valor promedio es aproximadamente 2 =n*theta, por lo tanto el valor promedio de theta es aproximadamente 2/n = 2/10 = .2
